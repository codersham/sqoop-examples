...
# Replace [URL] with the url of the Mysql server DB
...
--connect jdbc:mysql://[URL:PORT] --username sqoopuser -P

# Produce sqoop output and log to separate file in local file system
sqoop list-databases \
--connect jdbc:mysql://[URL] --username sqoopuser -password [password] \
